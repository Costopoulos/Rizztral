...
export interface AiDailyTransportCallbacks {
    ...
    onParticipantCountsUpdated?: (participantCounts: DailyParticipantCounts) => void;
    onLocalParticipantJoined?: (localParticipantId: string) => void;
}
...
export class TransportClient {
    ...
    private setupDailyEvents(call: DailyCall) {
        ...
        call.on("joined-meeting", () => {
            ...
            if (localParticipant) {
                this.callbacks.onLocalParticipantJoined?.(localParticipant.session_id);
            }
        ...
        call.on('participant-counts-updated', (event) => {
            logger.info("[TransportClient] participant-counts-updated:", event);
            // You can add a callback here if needed
            const participantCounts = event.participantCounts;
            console.log("[TransportClient] participantCounts:", participantCounts);
            this.callbacks.onParticipantCountsUpdated?.(participantCounts);
        });
