...
import { DailyParticipant, DailyParticipantCounts } from "@daily-co/daily-js";
...
type TransportContextType = {
  ...
  participantCounts: DailyParticipantCounts | null;
...
export const TransportProvider: React.FC<PropsWithChildren> = ({ children }) => {
  ...
  const [participantCounts, setParticipantCounts] = useState<DailyParticipantCounts | null>(null);
  ...
  const connectToAi = async () => {
    ...
    const client = new TransportClient({
      ...
      onParticipantJoined: (pid) => {
        ...
        setParticipants(prev => [...prev, {username: pid.user_name, id: pid.session_id, currently_speaking: false}]);
      ...
      onAudioTrackStarted: (participant: DailyParticipant, track: MediaStreamTrack) => {
        ...
        setParticipants(prev => prev.map(p => p.id === participant.session_id ? {...p, currently_speaking: true} : p));
      },
      onAudioTrackStopped: (participant: DailyParticipant, track: MediaStreamTrack) => {
        ...
        setParticipants(prev => prev.map(p => p.id === participant.session_id ? {...p, currently_speaking: false} : p));
      },
      onParticipantCountsUpdated: (participantCounts: DailyParticipantCounts) => {
        console.log("[TransportProvider] onParticipantCountsUpdated =>", participantCounts);
        setParticipantCounts(participantCounts);
      },
      onLocalParticipantJoined: (localParticipantId: string) => {
        console.log("[TransportProvider] onLocalParticipantJoined =>", localParticipantId);
        setLocalParticipantId(localParticipantId);
      }
    });
